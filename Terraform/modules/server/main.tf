...
resource "aws_security_group" "ivolve-sg" {
  name = "${var.mytag}-sg"
  description = "Security Group for ivolve jenkins server"
  vpc_id = var.vpc_id
  ingress {
...
resource "aws_instance" "ivolve_server" {
  ami = var.ami
  instance_type = var.instance_type
  subnet_id = var.public_subnet_id
  associate_public_ip_address = true
  vpc_security_group_ids = [aws_security_group.ivolve-sg.id]
...
resource "aws_cloudwatch_metric_alarm" "server_cpu" {
  ...
  statistic = "Average"
  threshold = "80"
  alarm_description = "CPU utilization has exceeded 80% for iVolve Jenkins server"
  alarm_actions = [aws_sns_topic.alarms.arn]
  ok_actions = [aws_sns_topic.alarms.arn]

  dimensions = {
    InstanceId = aws_instance.ivolve_server.id
  }
}
...
resource "aws_cloudwatch_metric_alarm" "server_mem" {
  ...
  statistic = "Average"
  threshold = "80"
  alarm_description = "Memory utilization has exceeded 80% for iVolve Jenkins server"
  alarm_actions = [aws_sns_topic.alarms.arn]
  ok_actions = [aws_sns_topic.alarms.arn]
...
resource "aws_cloudwatch_metric_alarm" "server_status" {
  ...
  statistic = "Maximum"
  threshold = "1"
  alarm_description = "iVolve Jenkins server has failed status check"
  alarm_actions = [aws_sns_topic.alarms.arn]
  ok_actions = [aws_sns_topic.alarms.arn]
